...
class EmailService:
    ...
    def send_email(self, destinatarios, assunto, corpo, html=False):
        ...
        try:
            ...
            if not all([smtp_server, port, sender_email, password, username]):
                logger.error("Configurações de e-mail incompletas. Verifique MAIL_SERVER, MAIL_PORT, MAIL_DEFAULT_SENDER, MAIL_PASSWORD e MAIL_USERNAME.")
                return False
            ...
            part = MIMEText(corpo, "html" if html else "plain")
            message.attach(part)
...
def formatar_email_notificacao(documento, dias_restantes, responsavel=None, link_documento=None):
    ...
    tipo_entidade = "Fazenda/Área" if getattr(documento.tipo_entidade, "value", "") == "Fazenda/Área" else "Pessoa"
    nome_entidade = getattr(documento, "nome_entidade", "")

    contexto = {
        "responsavel": responsavel,
        "nome_documento": getattr(documento, "nome", ""),
        "tipo_documento": getattr(getattr(documento, "tipo", None), "value", ""),
    ...
    try:
        ...
        # Se não houver contexto, cria um temporário
        if has_app_context():
            raise
...
def enviar_email_teste(destinatarios):
    """
    Envia e-mail de teste usando instância global.
    """
